...
export class MediaService {
  ...
  async list(options: ListMediaOptions, applicationId?: string) {
    try {
      logger.info({ options }, 'MediaService: list');
      return await this.repository.list({ ...options, applicationId });
    } catch (error) {
      logger.error({ err: error }, 'MediaService Error in list:');
      throw new ApiError(500, SERVICE_ERRORS.LIST_MEDIA_FAILED);
    }
  }
  async getById(id: string, applicationId?: string): Promise<MediaAssetRecord> {
    try {
      logger.info({ id }, 'MediaService: getById');
      const asset = await this.repository.findById(id, applicationId);
      if (!asset) {
  ...
  async getByStorageKey(
    ...
    try {
      ...
      const asset = await this.repository.findByStorageKey(key, applicationId);
      if (!asset) {
