  ...
  async write(
    ...
    try {
      ...
      if (isNodeErrnoException(error) && error.code === 'ENOENT') {
        // The directory might have been deleted while the server was running.
        // Reset the flag and try recreating it once.
        ...
        try {
          ...
          throw new StorageWriteError(
            `Failed to write "${key}" to local storage after recreating directory.`,
            { cause: retryError },
          );
        }
      }
